...
const Bookings = () => {
  ...
  const safeFilteredBookings = Array.isArray(bookings) ? filteredBookings() : [];
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex mb-6 border-b">
        <button 
          className={`py-2 px-4 mr-4 ${filter === 'all' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500'}`}
          onClick={() => setFilter('all')}
        >
          All Bookings
        </button>
        <button 
          className={`py-2 px-4 mr-4 ${filter === 'upcoming' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500'}`}
          onClick={() => setFilter('upcoming')}
        >
          Upcoming
        </button>
        <button 
          className={`py-2 px-4 ${filter === 'past' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500'}`}
      ...
      {loading ? (
        ...
        <div className="text-center py-8">
          ...
          <Link to="/rooms" className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
            Browse Rooms
          </Link>
        </div>
      ) : (
        <div className="grid gap-6">
          {safeFilteredBookings.map(booking => (
            <div key={booking._id} className="bg-white shadow-md rounded-lg overflow-hidden border">
              <div className="md:flex">
                ...
                <div className="p-6 flex-grow">
                  <div className="flex flex-wrap justify-between items-start mb-2">
                    <h2 className="text-xl font-bold">Room {booking.room?.roomNumber || 'Unknown'}</h2>
                  ...
                  <div className="mb-4">
                    ...
                    <p className="text-gray-600">
                      {calculateNights(booking.checkInDate, booking.checkOutDate)} nights, {booking.guests} {booking.guests === 1 ? 'guest' : 'guests'}
                    </p>
                  </div>
